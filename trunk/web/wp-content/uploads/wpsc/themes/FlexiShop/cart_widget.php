...
<?php if(wpsc_cart_item_count() > 0): ?>
  <span class='items'>
		<span class='numberitems'>
			<?php echo __('Number of items', 'flexishop'); ?>:
		</span>
...
<?php if( (wpsc_cart_tax(false) >0) && !wpsc_cart_show_plus_postage()) : ?>
	...
	<form action='' method='post' class='wpsc_empty_the_cart'>
		...
		<span class='emptycart'>
			<a target='_parent' href='<?php echo htmlentities(add_query_arg('wpsc_ajax_action', 'empty_cart', remove_query_arg('ajax')), ENT_QUOTES); ?>'><?php echo __('Empty your cart', 'flexishop'); ?></a>
		</span>                                                                                             
	</form>
	
	<span class='gocheckout'><a target='_parent' href='<?php echo get_option('shopping_cart_url'); ?>'><?php echo __('Go to Checkout', 'flexishop'); ?></a></span>
<?php else: ?>
