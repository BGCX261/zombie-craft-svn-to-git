    ...
    public void initGui()
    {
        ...
        for(int k = 0; k < j; k++)
        {
            ...
            if(!enumoptions.getEnumFloat())
            {
            	...
            	if (enumoptions.returnEnumOrdinal() == 8) {
            		if (ModLoader.getMinecraftInstance().theWorld != null) {
	            		GuiSmallButton temp = new GuiSmallButton(enumoptions.returnEnumOrdinal(), (width / 2 - 155) + (i % 2) * 160, height / 6 + 24 * (i >> 1), enumoptions, "-Game in Progress-");
	            		temp.enabled = false;
	            		controlList.add(temp);
            		} else {
            			GuiSmallButton temp = new GuiSmallButton(enumoptions.returnEnumOrdinal(), (width / 2 - 155) + (i % 2) * 160, height / 6 + 24 * (i >> 1), enumoptions, options.getKeyBinding(enumoptions));
                		controlList.add(temp);
            		}
            	} else {
            		GuiSmallButton temp = new GuiSmallButton(enumoptions.returnEnumOrdinal(), (width / 2 - 155) + (i % 2) * 160, height / 6 + 24 * (i >> 1), enumoptions, options.getKeyBinding(enumoptions));
