...
public class Chunk
{
    ...
    public void generateHeightMap()
    {
        ...
        for(int j = 0; j < 16; j++)
        {
            ...
            {
                int l = 127;
                for(int i1 = j << 11 | k << 7; l > 0 && Block.lightOpacity[func_27445_translate256(blocks[(i1 + l) - 1])] == 0; l--) { }
    ...
    public void func_1024_c()
    {
        ...
        for(int l = 0; l < 16; l++)
        {
            for(int j = 0; j < 16; j++)
            {
                int j1 = 127;
                int k1;
                for(k1 = l << 11 | j << 7; j1 > 0 && Block.lightOpacity[func_27445_translate256(blocks[(k1 + j1) - 1])] == 0; j1--) { }
                heightMap[j << 4 | l] = (byte)j1;
                if(j1 < i)
                {
                    i = j1;
                }
                if(!worldObj.worldProvider.field_6478_e)
                {
                    ...
                    do
                    {
                        l1 -= Block.lightOpacity[func_27445_translate256(blocks[k1 + i2])];
                        if(l1 > 0)
                        {
                            skylightMap.setNibble(l, i2, j, l1);
                        }
                    } while(--i2 > 0 && l1 > 0);
                }
            }
    ...
    private void func_1003_g(int i, int j, int k)
    {
        ...
        for(int j1 = i << 11 | k << 7; i1 > 0 && Block.lightOpacity[func_27445_translate256(blocks[(j1 + i1) - 1])] == 0; i1--) { }
        if(i1 == l)
        {
            return;
        }
    ...
    {
        return func_27445_translate256(blocks[i << 11 | k << 7 | j]);
    }

    public boolean setBlockIDWithMetadata(int i, int j, int k, int l, int i1)
    {
        int j1 = func_27445_translate256(l);
        int k1 = heightMap[k << 4 | i] & 0xff;
        int l1 = func_27445_translate256(blocks[i << 11 | k << 7 | j]) & 0xff;
        ...
        int j2 = zPosition * 16 + k;
        blocks[i << 11 | k << 7 | j] = (byte)func_27444_translate128(j1);
        ...
        if(!worldObj.worldProvider.field_6478_e)
        {
            ...
            worldObj.func_616_a(EnumSkyBlock.Sky, i2, j, j2, i2, j, j2);
        }
        worldObj.func_616_a(EnumSkyBlock.Block, i2, j, j2, i2, j, j2);
        func_996_c(i, k);
        data.setNibble(i, j, k, i1);
        if(l != 0)
        {
    ...
    {
        int i1 = func_27445_translate256(l);
        int j1 = heightMap[k << 4 | i] & 0xff;
        int k1 = func_27445_translate256(blocks[i << 11 | k << 7 | j]) & 0xff;
        if(k1 == l)
        {
            return false;
        }
        ...
        blocks[i << 11 | k << 7 | j] = (byte)func_27444_translate128(i1);
        if(k1 != 0)
        {
    ...
    public int setChunkData(byte abyte0[], int i, int j, int k, int l, int i1, int j1, 
            int k1)
    {
        for(int l4 = i; l4 < l; l4++)
        {
            for(int l1 = k; l1 < j1; l1++)
            {
                int l2 = l4 << 11 | l1 << 7 | j;
                int l3 = i1 - j;
                System.arraycopy(abyte0, k1, blocks, l2, l3);
                k1 += l3;
            }
        ...
        for(int i5 = i; i5 < l; i5++)
        {
            ...
            {
                int i3 = (i5 << 11 | i2 << 7 | j) >> 1;
                int i4 = (i1 - j) / 2;
                System.arraycopy(abyte0, k1, data.data, i3, i4);
                k1 += i4;
            }

        }

        for(int j5 = i; j5 < l; j5++)
        {
            for(int j2 = k; j2 < j1; j2++)
            {
                int j3 = (j5 << 11 | j2 << 7 | j) >> 1;
                int j4 = (i1 - j) / 2;
                System.arraycopy(abyte0, k1, blocklightMap.data, j3, j4);
                k1 += j4;
            }

        }

        for(int k5 = i; k5 < l; k5++)
        {
            for(int k2 = k; k2 < j1; k2++)
            {
                int k3 = (k5 << 11 | k2 << 7 | j) >> 1;
                int k4 = (i1 - j) / 2;
                System.arraycopy(abyte0, k1, skylightMap.data, k3, k4);
                k1 += k4;
            }
    ...
    public static int func_27444_translate128(int i)
    {
        if(i > 127)
        {
            return -129 + (i - 127);
    ...
    public static int func_27445_translate256(int i)
    {
        if(i < 0)
        {
            return i + 256;
