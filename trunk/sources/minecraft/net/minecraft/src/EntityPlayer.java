...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    public EntityPlayer(World world)
    {
        ...
        field_9371_f = 0;
        //score = 0;
        score = world.field_22145_q.startScore;
        isSwinging = false;
        swingProgressInt = 0;
        damageRemainder = 0;
        fishEntity = null;
    ...
    public void addStartLoc() {
    	worldObj.field_22145_q.startPosX[worldObj.field_22145_q.startPosCount] = posX;
    	worldObj.field_22145_q.startPosY[worldObj.field_22145_q.startPosCount] = posY;
    	worldObj.field_22145_q.startPosZ[worldObj.field_22145_q.startPosCount] = posZ;
    	worldObj.field_22145_q.startPosCount++;
    }

    public void onUpdate()
    {
    	nearDoor = false;
    	for (int x = ((int)(posX-1.0))-1; x <= ((int)(posX+0.0))+1; x++) {
    		for (int z = ((int)(posZ-1.0))-1; z <= ((int)(posZ+0.0))+1; z++) {
    			if (((BlockBarricade)mod_ZombieCraft.barricadeS5).isBarricade(worldObj,x,(int)posY,z)) {
    				barrierX = x;
    				barrierY = (int)posY;
    				barrierZ = z;
    				nearDoor = true;
    	...
    	if (canCharge && Keyboard.isKeyDown(mod_ZombieCraft.mc.gameSettings.keyBindCharge.keyCode) && chargeCooldown <= 0)
		{
    		//System.out.println("charge go");
    		worldObj.playSoundEffect(posX, posY, posZ, "sdkzc.Charge", 2F, 1.0F);
    		chargeCooldown = 20*15;
    		chargeTimer = 20;
		}
    	...
    	if (chargeTimer > 0) {
    		...
    		/*motionX = 0.0F;
    		motionZ = 0.0F;*/
    		setRelVel(this,0.0F,(float)motionY,1.0F,0.7F+(speedCola ? 0.3F : 0.0F));
    	}
    	
    	if (pushDelay > 0) { pushDelay--; }
    ...
    {
		if(System.currentTimeMillis() - doubleScoreTimer >= 15000) {
			doubleScore = false;
		}
		if(System.currentTimeMillis() - instaKillTimer >= 15000) {
			worldObj.instaKill = false;
		}
        if(health < 20 && (ticksExisted % 80) * 12 == 0)
        {
            heal(1);
        }
    ...
    public void onDeath(Entity entity, World world, EntityPlayer entityplayer)
    {
        super.onDeath(entity);
        setSize(0.2F, 0.2F);
        setPosition(posX, posY, posZ);
        motionY = 0.10000000149011612D;
		world.playSoundEffect(entityplayer.posX, entityplayer.posY, entityplayer.posZ, "sdkzc.end", 2F, 1.0F);
    ...
    public void addToPlayerScore(Entity entity, int i)
    {
		...
		float dFactor = 1.0F;
		if (worldObj.difficultySetting == 1) dFactor = 1.5F;
		if (worldObj.difficultySetting == 3) dFactor = 0.75F;
    ...
    public boolean attackEntityFrom(Entity entity, int i)
    {
        ...
        if (entity != null && !(entity instanceof EntityPlayer) && !(entity instanceof EntityFriend)) {
	        if (canShock && shockCooldown <= 0) {
	        	
	        	System.out.println("exstatic go");
	        ...
	        if (shockTimer > 0 || godMode) {
	        	...
	        	entity.attackEntityFrom(this,i*2);
	        	
	        	double d = posX - entity.posX;
	            double d1;
	            for(d1 = posZ - entity.posZ; d * d + d1 * d1 < 0.0001D; d1 = (Math.random() - Math.random()) * 0.01D)
	            {
	                d = (Math.random() - Math.random()) * 0.01D;
	            }
	            if (entity instanceof EntityLiving) {
	            	((EntityLiving)entity).knockBack(this,0,d,d1,2.0F);
	            	entity.motionY += 0.5F;
	            }
	            return false;
	        }
	...
	public boolean speedCola;
	public boolean doubleScore;
	public long doubleScoreTimer;
	public long instaKillTimer;
