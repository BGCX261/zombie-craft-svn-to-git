    ...
    implements IBlockAccess
{
    ...
    public MovingObjectPosition rayTraceBlocks(Vec3D vec3d, Vec3D vec3d1)
    {
    	...
    	return rayTraceBlocks_doorEx(vec3d, vec3d1, false);
    }
    
    public MovingObjectPosition rayTraceBlocks_def(Vec3D vec3d, Vec3D vec3d1, boolean flag)
    {
    ...
    public MovingObjectPosition rayTraceBlocks_do(Vec3D vec3d, Vec3D vec3d1, boolean flag)
    {
        ...
        for(int k1 = 200; k1-- >= 0;)
        {
            ...
            Block block = Block.blocksList[l1];
            //if door or spawner
            if ((block instanceof BlockDoor) || (l1 >= 100 && l1 < 140 && l1 != 123)) {
            	continue;
            }
            if(l1 > 0 && block.canCollideCheck(i2, flag))
            {
    ...
    public MovingObjectPosition rayTraceBlocks_doorEx(Vec3D vec3d, Vec3D vec3d1, boolean flag)
    {
        if(Double.isNaN(vec3d.xCoord) || Double.isNaN(vec3d.yCoord) || Double.isNaN(vec3d.zCoord))
        {
            return null;
        ...
        for(int k1 = 200; k1-- >= 0;)
        {
            ...
            Block block = Block.blocksList[l1];
            //if door or spawner
            if (((block instanceof BlockDoor) || (l1 >= 100 && l1 < 140 && l1 != 123)) && !devMode) {
            	continue;
            }
            if(l1 > 0 && block.canCollideCheck(i2, flag))
            {
            ...
            if (i1-1 > 0) {
	            ...
	            l1 = getBlockId(l, i1-1, j1);
	            i2 = getBlockMetadata(l, i1-1, j1);
	            block = Block.blocksList[l1];
	            
	            if(l1 == 85 && l1 > 0 && block.canCollideCheck(i2, flag))
	            {
	            	
	                MovingObjectPosition movingobjectposition = block.collisionRayTrace(this, l, i1-1, j1, vec3d, vec3d1);
	                if(movingobjectposition != null)
	                {
	                    return movingobjectposition;
	                }
    ...
    protected void func_4080_j()
    {
        ...
        for(Iterator iterator = field_9427_K.iterator(); iterator.hasNext();)
        {
            ...
            for(int l1 = 0; l1 < 80; l1++)
            {
                ...
                int k3 = k2 >> 8 & 0xf;
                int i4 = k2 >> 16 & 0x7f;
                int j4 = Chunk.func_27445_translate256(chunk.blocks[i3 << 11 | k3 << 7 | i4]);
	...
	public int debrisY;
	public int debrisZ;
	public ArrayList<EntityLiving> mobArr = new ArrayList<EntityLiving>();
