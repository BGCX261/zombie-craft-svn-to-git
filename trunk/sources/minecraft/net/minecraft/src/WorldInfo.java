...
public class WorldInfo
{
    ...
    public WorldInfo(NBTTagCompound nbttagcompound)
    {
        ...
        power = nbttagcompound.getBoolean("Power");
        startScore = nbttagcompound.getInteger("startScore");
        
        startPosCount = 0;
        for (int i = 0; i < 10; i++) {
        	if(nbttagcompound.hasKey((new StringBuilder()).append("startPosX_").append(i).toString())) {
        		startPosCount++;
        	}
        }
        
        startPosX = new double[10];
        startPosY = new double[10];
        startPosZ = new double[10];
        
        for (int i = 0; i < startPosCount; i++) {
        	startPosX[i] = nbttagcompound.getDouble((new StringBuilder()).append("startPosX_").append(i).toString());
        	startPosY[i] = nbttagcompound.getDouble((new StringBuilder()).append("startPosY_").append(i).toString());
        	startPosZ[i] = nbttagcompound.getDouble((new StringBuilder()).append("startPosZ_").append(i).toString());
        }
    ...
    public WorldInfo(WorldInfo worldinfo)
    {
		...
		power = worldinfo.power;
		startScore = worldinfo.startScore;
    }
    ...
    private void func_22291_a(NBTTagCompound nbttagcompound, NBTTagCompound nbttagcompound1)
    {
        ...
        nbttagcompound.setBoolean("Power", power);
        nbttagcompound.setInteger("startScore", startScore);
        
        for (int i = 0; i < startPosCount; i++) {
        	nbttagcompound.setDouble((new StringBuilder()).append("startPosX_").append(i).toString(), startPosX[i]);
        	nbttagcompound.setDouble((new StringBuilder()).append("startPosY_").append(i).toString(), startPosY[i]);
        	nbttagcompound.setDouble((new StringBuilder()).append("startPosZ_").append(i).toString(), startPosZ[i]);
        }
