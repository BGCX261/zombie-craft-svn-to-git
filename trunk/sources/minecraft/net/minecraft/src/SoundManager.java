...
public class SoundManager
{
    ...
    public void loadSoundSettings(GameSettings gamesettings)
    {
        ...
        loadModAudio("resources/mod/sound", soundPoolSounds);
        loadModAudio("resources/mod/streaming", soundPoolStreaming);
        loadModAudio("resources/mod/music", soundPoolMusic);
    }

    private static void loadModAudio(String s, SoundPool soundpool)
    {
        File file = new File(Minecraft.getMinecraftDir(),s);//Minecraft.getAppDir(s);
        try
        {
        ...
        if(file1.exists() || file1.mkdirs())
        {
            ...
            if(afile != null && afile.length > 0)
            {
                for(int i = 0; i < afile.length; i++)
                {
                    ...
                    if(afile[i].isFile() && !afile[i].getName().startsWith("."))
                    {
