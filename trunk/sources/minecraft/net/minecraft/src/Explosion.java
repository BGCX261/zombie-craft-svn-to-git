...
public class Explosion
{
    ...
    public Explosion(World world, Entity entity, double d, double d1, double d2, float f, boolean b1)
    {
        field_12257_a = false;
    ...
    public void func_12247_b()
    {
        ...
        for(int i = arraylist.size() - 1; i >= 0; i--)
        {
            ...
            for(int j1 = 0; j1 < 1; j1++)
            {
				...
				EntityExplodeFX eFX = new EntityExplodeFX(worldObj, (d + explosionX * 1.0D) / 2D, (d1 + explosionY * 1.0D) / 2D, (d2 + explosionZ * 1.0D) / 2D, d3, d4, d5);
				if(silent) {
					eFX.particleRed = 0F;
					eFX.particleBlue = 0.5F;
					eFX.particleGreen = 0.5F;
				}
            ...
            if(i1 > 0)
            {
                //Block.blocksList[i1].dropBlockAsItemWithChance(worldObj, j, k, l, worldObj.setBlockMetadata(j, k, l), 0.3F);
                if (worldObj.getBlockId(j, k, l) == mod_ZombieCraft.cobbleBreak.blockID)
				{
