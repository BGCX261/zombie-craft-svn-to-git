...
{
	private Block blockLogo = Block.stone;
    public GuiMainMenu()
    {
    	//Modloader.getMinecraftInstance().texturePackList.forceDefaultTexturePackWithoutSave();
        updateCounter = 0.0F;
    ...
    public void initGui()
    {
        ...
        int i = height / 4 + 48;
        controlList.add(new GuiButton(1, width / 2 - 100, i, "ZombieCraft"));
		GuiButton createMapButton;
        controlList.add(createMapButton = new GuiButton(2, width / 2 - 100, i + 24, "Create a map"));
		//createMapButton.enabled = false;
        //controlList.add(new GuiButton(3, width / 2 - 100, i + 48, "About ZombieCraft"));
    ...
    protected void actionPerformed(GuiButton guibutton)
    {
        ...
        if(guibutton.id == 1)
        {
            mc.displayGuiScreen(new GuiSelectZombieWorld(this));
        }
        if(guibutton.id == 2)
        {
        ...
        for(int l = 0; l < 3; l++)
        {
            ...
            for(int i1 = 0; i1 < minecraftLogo.length; i1++)
            {
                for(int j1 = 0; j1 < minecraftLogo[i1].length(); j1++)
                {
					...
					renderblocks.func_1238_a(blockLogo, f3);
                    GL11.glPopMatrix();
                }
