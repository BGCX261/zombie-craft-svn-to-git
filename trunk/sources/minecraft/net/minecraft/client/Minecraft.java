    ...
    implements Runnable
{
    ...
    public Minecraft(Component component, Canvas canvas, MinecraftApplet minecraftapplet, int i, int j, boolean flag)
    {
        ...
        textureWaterFX = new TextureWaterFX(this);
        textureLavaFX = new TextureLavaFX(this);
        running = true;
        debug = "";
    ...
    public void startGame() throws LWJGLException
    {
        ...
        field_22008_V = new SaveConverterMcRegion(new File(mcDataDir, "zctemp"));
        ...
        renderEngine.registerTextureFX(new TexureWaterFlowFX(this));
        renderEngine.registerTextureFX(new TextureLavaFlowFX(this));
    ...
    public void runTick()
    {
        ...
        if(currentScreen == null || currentScreen.field_948_f)
        {
            ...
            do
            {
                ...
                if(Keyboard.getEventKeyState())
                {
                    ...
                    } else
                    {
						...
						} else if (thePlayer.inventory.mainInventory[3] != null)
						{
							for(int i = 0; i < 4; i++)
							{
								if(Keyboard.getEventKey() == 2 + i)
						...
						} else if (thePlayer.inventory.mainInventory[2] != null)
						{
							for(int i = 0; i < 3; i++)
							{
								if(Keyboard.getEventKey() == 2 + i)
            ...
            {
            	// TODO
            	if(!sdkZombieCraftNotFound)
            	{
                	try
                    {
                		if(this.sdkZombieCraftHandleMouse0 == null)
                		{
        	        		Class sdkGuns;
        	        		try
        	        		{
        	        			sdkGuns = Class.forName("mod_SdkZombieCraft");
        	        		}
        	        		catch (Exception e)
        	        		{
        	        			sdkGuns = Class.forName("net.minecraft.src.mod_SdkZombieCraft");
        	        		}
        	        		
        	        		sdkZombieCraftHandleMouse0 = sdkGuns.getDeclaredMethod("handleMouse0", new Class[] { boolean.class });
        	        		sdkZombieCraftOverrideMouseCheck0 = sdkGuns.getDeclaredMethod("overrideMouseCheck0", new Class[] { int.class });
        	        		sdkZombieCraftHandleMouse1 = sdkGuns.getDeclaredMethod("handleMouse1", new Class[] { boolean.class });
        	        		sdkZombieCraftOverrideMouseCheck1 = sdkGuns.getDeclaredMethod("overrideMouseCheck1", new Class[] { int.class });
                		}
                		
                		if(!(boolean)(Boolean)sdkZombieCraftHandleMouse0.invoke(null, new Object[] { Mouse.isButtonDown(0) }))
        	            	if(Mouse.isButtonDown(0) && ((boolean)(Boolean)sdkZombieCraftOverrideMouseCheck0.invoke(null, new Object[] { ticksRan - field_6302_aa })
        	            			|| ((float)(ticksRan - field_6302_aa) >= timer.ticksPerSecond / 4F && field_6289_L)))
        	                {
        	            		clickMouse(0);
        	                    field_6302_aa = ticksRan;
        	                }
                    	
                    	if(!(boolean)(Boolean)sdkZombieCraftHandleMouse1.invoke(null, new Object[] { Mouse.isButtonDown(1) }))
        	                if(Mouse.isButtonDown(1) && ((boolean)(Boolean)sdkZombieCraftOverrideMouseCheck1.invoke(null, new Object[] { ticksRan - field_6302_aa })
        	                		|| ((float)(ticksRan - field_6302_aa) >= timer.ticksPerSecond / 4F && field_6289_L)))
        	                {
        	                	clickMouse(1);
        	                    field_6302_aa = ticksRan;
                    ...
                    { 
                    	System.out.println("mod_SdkGuns not found in Minecraft (A). Ignore this exception if you do not have it installed.");
                    	e.printStackTrace();
                    	sdkZombieCraftNotFound = true;
                    }
            	}
            	else
            	{
    ...
    // TODO
    private static boolean sdkZombieCraftNotFound = false;
    private static Method sdkZombieCraftHandleMouse0 = null;
    private static Method sdkZombieCraftOverrideMouseCheck0 = null;
    private static Method sdkZombieCraftHandleMouse1 = null;
    private static Method sdkZombieCraftOverrideMouseCheck1 = null;
}
